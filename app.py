...
# ======================================================
# ODD_EVEN_BETS — ระบบเดิมพันคี่/คู่
# key = bet_id (message_id ของโพสต์ต้นทาง)
# value = {
#   "bet_id", "round_id", "chat_id", "camp_name",
#   "maker_id", "taker_id", "amount", "maker_choice",
#   "status": "open" | "matched" | "settled" | "cancelled",
#   "result_value", "winning_choice", "winner_id",
#   "created_at", "matched_at", "settled_at",
# }
# ======================================================
ODD_EVEN_BETS = {}

# ======================================================
# ระบบเล่นบ่ลำ/ลำ (LADDER_BETS)
# โครงสร้างเหมือน ODD_EVEN_BETS แต่สำหรับเล่นบ่ลำ/ลำ
# ======================================================
LADDER_BETS = {}
...
def admin_command_needs_explicit_base(text: str, chat_id: str = None) -> bool:
    ...
    # 11.5) สรุปผล บ่ลำ/ลำ (เฉพาะแอดมิน)
    # คำสั่ง: สรุปผล ลำ / สรุปผล บ่ลำ
    ladder_settle_m = re.match(r"^(?:สรุปผล|สรุป)\s+(ลำ|บ่ลำ)$", text.strip())
    if ladder_settle_m:
        ...
        if not is_front_chat(event):
            reply_text(event.reply_token, front_room_block_text("สรุปผล บ่ลำ/ลำ"))
            return
            
        result_choice = ladder_settle_m.group(1)
        
        # ตรวจสอบว่ามีรอบอยู่หรือไม่
        if STATE.get("round_id") is None:
            reply_text(event.reply_token, "ยังไม่มีรอบ กรุณาเปิดรอบก่อน")
            return
            
        if STATE.get("settled"):
            reply_text(event.reply_token, "รอบนี้แจ้งผลหลักไปแล้ว ไม่สามารถสรุปผล บ่ลำ/ลำ ย้อนหลังได้")
            return
            
        with STATE_LOCK:
            res = settle_ladder_bets_for_round(STATE["round_id"], result_choice)
            
        reply_text(
            event.reply_token,
            f"✅ สรุปผล บ่ลำ/ลำ สำเร็จ\n\n"
            f"ผลที่ออก: {result_choice}\n"
            f"จับคู่สำเร็จ: {res['matched_count']} แผล\n"
            f"ผู้โพสต์ชนะ: {res['maker_win_count']} แผล\n"
            f"ผู้ติดชนะ: {res['taker_win_count']} แผล\n"
            f"จ่ายเงินรางวัล: {res['total_payout']:,} บาท\n"
            f"คืนทุนโพสต์ไม่มีคนติด: {res['total_refund']:,} บาท"
        )
        return
...
def save_user_db():
    ...
    data = {
        ...
        "users": USERS,
        "odd_even_bets": ODD_EVEN_BETS,
        "ladder_bets": LADDER_BETS,  # บันทึก ODD_EVEN_BETS ลงไฟล์ users.json
...
def parse_odd_even_offer(text: str):
    ...
    ตัวอย่าง: คี่500 / คู่1000 / คี่ 500 / คู่ 1000
    คืน {"choice": "คี่"/"คู่", "amount": int} หรือ None
    """
    clean = re.sub(r"\s+", "", (text or "").strip())
    m = re.match(r"^(คี่|คู่)(\d+)$", clean)
    if not m:
        return None
    amount = int(m.group(2))
    if amount <= 0:
        return None
    return {"choice": m.group(1), "amount": amount}


def create_odd_even_post(event, oe_offer: dict):
    """
    ลูกค้าโพสต์ คี่500 หรือ คู่500
    เปิดได้ทุกเวลา ไม่ขึ้นกับสถานะรอบ (เปิด/ปิด/ยังไม่แจ้งผล)
    ต้องอยู่ในรอบที่มี round_id และยังไม่ settled
    """
    user_id = event.source.user_id
    user = ensure_user_from_event(event)
    ...
    # ต้องมีรอบอยู่ (เปิด หรือปิดแล้วแต่ยังไม่แจ้งผล)
    active_state = None
    for st in ROUNDS.values():
        if not isinstance(st, dict):
            continue
        chat_id_match = (not st.get("chat_id")) or st.get("chat_id") == get_current_chat_id(event)
        if st.get("round_id") and not st.get("settled") and chat_id_match:
            active_state = st
            break

    if not active_state:
        return "ยังไม่มีรอบ ไม่สามารถเล่นคี่/คู่ได้"

    amount = oe_offer["amount"]
    if user_credit_amount(user) < amount:
        return insufficient_credit_warning(user, amount, play_text=f"{oe_offer['choice']}{amount:,}", action="เดิมพันคี่/คู่")

    bet_id = get_message_id(event)
    if not bet_id:
        return "ระบบไม่พบ message id ของโพสต์นี้"

    ODD_EVEN_BETS[bet_id] = {
        "bet_id": bet_id,
        "round_id": active_state["round_id"],
        "chat_id": active_state.get("chat_id"),
        "camp_name": active_state.get("camp_name"),
        "maker_id": user_id,
        "taker_id": None,
        "amount": amount,
        "maker_choice": oe_offer["choice"],
        "status": "open",
        "result_value": None,
        "winning_choice": None,
        "winner_id": None,
...
def handle_odd_even_confirm(event, quoted_message_id, requested_amount=None):
    ...
    taker_id = event.source.user_id
    if taker_id == bet["maker_id"]:
...
def settle_odd_even_bets_for_round(round_id: str, result_value: int):
    """
    เรียกหลัง settle_round สำเร็จ
    ตัดสินบิลคี่/คู่ที่ matched ในรอบนั้น ๆ ด้วยตัวเลขผลจากแอดมิน
    """
    if not round_id:
        return
...
def settle_odd_even_bets_all_jow_for_round(round_id: str, reason: str):
    """
    กรณีแจ้งผลคืนทุน (จาวทุกแผล / บั้งไฟหาย) คืนเครดิตทุกบิลคี่/คู่ในรอบ
    """
    if not round_id:
        return
...
def _cancel_unmatched_odd_even_for_round(round_id: str, reason: str = "รอบปิด"):
    """คืนเครดิตโพสต์คี่/คู่ที่ยังไม่มีคนติด (status=open) ในรอบนี้"""
    for bet in ODD_EVEN_BETS.values():
        if bet.get("round_id") == round_id and bet.get("status") == "open":
            maker_id = bet["maker_id"]
            amount = bet["amount"]
            maker = USERS.get(maker_id)
            if maker:
            ...
            push_text_async(
                maker_id,
                f"⚠️ ยกเลิก (คี่/คู่) — ไม่มีคนมาติด\n"
                f"เหตุผล: {reason}\n"
...
def parse_ladder_offer(text: str):
    """
    รับคำสั่ง บ่ลำ<amount> หรือ ลำ<amount> และคำพ้องเสียงต่างๆ
    บ่ลำ มี: บ่ลำ / บ่ลำแน่ / บ่ลำแน่นิ / บ่ลำดาบ
    ลำ มี: ลำ / ลำแน่ / ลำแน่นิ / ลำดาบ
    ตัวอย่าง: บ่ลำ500 / ลำ1000 / บ่ลำแน่ 500 / ลำดาบ 1000
    คืน {"choice": "บ่ลำ"/"ลำ", "amount": int, "raw_choice": str} หรือ None
    """
    clean = re.sub(r"\s+", "", (text or "").strip())
    
    no_ladder_keywords = ["บ่ลำแน่", "บ่ลำแน่นิ", "บ่ลำดาบ", "บ่ลำ"]
    ladder_keywords = ["ลำแน่", "ลำแน่นิ", "ลำดาบ", "ลำ"]
    
    for kw in no_ladder_keywords:
        if clean.startswith(kw):
            amount_str = clean[len(kw):]
            if amount_str.isdigit():
                amount = int(amount_str)
                if amount > 0:
                    return {"choice": "บ่ลำ", "amount": amount, "raw_choice": kw}
                    
    for kw in ladder_keywords:
        if clean.startswith(kw):
            amount_str = clean[len(kw):]
            if amount_str.isdigit():
                amount = int(amount_str)
                if amount > 0:
                    return {"choice": "ลำ", "amount": amount, "raw_choice": kw}
                    
    return None


def create_ladder_post(event, ladder_offer: dict):
    """
    ลูกค้าโพสต์ บ่ลำ500 หรือ ลำ500
    เล่นได้เฉพาะหลังเปิดรอบ (ถ้าปิดรอบจะเล่นไม่ได้)
    ต้องอยู่ในรอบที่มี round_id และยังไม่ settled และ opened เป็น True เท่านั้น
    """
    user_id = event.source.user_id
    user = ensure_user_from_event(event)
    ...
    for st in ROUNDS.values():
        if not isinstance(st, dict):
            continue
        chat_id_match = (not st.get("chat_id")) or st.get("chat_id") == get_current_chat_id(event)
        if st.get("round_id") and st.get("opened") and not st.get("settled") and chat_id_match:
            active_state = st
            break

    if not active_state:
        return "❌ ตอนนี้ปิดรอบแล้ว หรือยังไม่มีรอบ ไม่สามารถเล่น บ่ลำ/ลำ ได้"

    amount = ladder_offer["amount"]
    if user_credit_amount(user) < amount:
        return insufficient_credit_warning(user, amount, play_text=f"{ladder_offer['raw_choice']}{amount:,}", action="เดิมพันบ่ลำ/ลำ")
    ...
    LADDER_BETS[bet_id] = {
        ...
        "maker_id": user_id,
        "taker_id": None,
        "taker_reply_message_id": None,
        "amount": amount,
        "maker_choice": ladder_offer["choice"],
        "raw_choice": ladder_offer["raw_choice"],
        "status": "open",
        "result_value": None,
        "winning_choice": None,
        "winner_id": None,
    ...
    user["credit"] -= amount
    save_user_db()
    save_round_backup_db(reason="ladder_post_created")

    # ส่ง Flex ยืนยันการโพสต์สำเร็จในห้องกลุ่ม
    # เลียนแบบโพสต์ คี่/คู่
    reply_flex(
        event.reply_token,
        f"โพสต์ บ่ลำ/ลำ {amount:,}",
        oe_post_flex(LADDER_BETS[bet_id], user_id) # สามารถใช้ oe_post_flex ได้เพราะโครงสร้างข้อมูลเหมือนกัน
    )
    return None


def is_ladder_post_message_id(message_id: str) -> bool:
    """ตรวจว่า message_id นี้เป็นโพสต์ บ่ลำ/ลำ ที่ยังเปิดอยู่ (รอคนติด)"""
    bet = LADDER_BETS.get(message_id)
    return bool(bet and bet.get("status") == "open")


def find_ladder_pending_taker_by_reply_id(reply_message_id: str) -> dict:
    """หาโพสต์ บ่ลำ/ลำ ที่มี pending taker reply message id ตรงกัน"""
    for bet in LADDER_BETS.values():
        if (
            bet.get("status") == "pending_taker"
            and bet.get("taker_reply_message_id") == reply_message_id
        ):
...
def is_ladder_pending_taker_message_id(message_id: str) -> bool:
    """ตรวจว่า message_id นี้เป็นข้อความ ติด ของ taker ที่รอ maker ยืนยัน"""
    return find_ladder_pending_taker_by_reply_id(message_id) is not None


def handle_ladder_confirm(event, quoted_message_id, requested_amount=None):
    """
    Flow 2 ขั้นตอน (เหมือนระบบแผลเดิม):
    ขั้น 1: B reply โพสต์ บ่ลำ/ลำ ของ A ด้วย ต/ติด -> บันทึก pending_taker, บอทเงียบในกลุ่ม รอ A ยืนยัน
    ขั้น 2: A reply ข้อความ ต/ติด ของ B ด้วย ต/ติด -> จับคู่สำเร็จ, หักเครดิต B, ส่ง Flex ยืนยันทั้งสองฝ่าย
    """
    if not quoted_message_id:
        return None
    ...
    # ขั้น 2: A ยืนยัน taker ที่ pending
    # ──────────────────────────────────────────────
    pending_bet = find_ladder_pending_taker_by_reply_id(quoted_message_id)
    if pending_bet:
        user_id = event.source.user_id
        ...
        maker_choice = pending_bet["maker_choice"]
        taker_choice = "ลำ" if maker_choice == "บ่ลำ" else "บ่ลำ"
        pending_bet["taker_choice"] = taker_choice
        pending_bet["status"] = "matched"
        pending_bet["matched_at"] = now_text()
        pending_bet["order_no"] = get_next_order_no()
        ...
        save_user_db()
        save_round_backup_db(reason="ladder_matched")

        # ส่ง Flex หาทั้งสองฝ่ายแบบ async
        push_flex_async(
            pending_bet["maker_id"],
            "จับคู่สำเร็จ (บ่ลำ/ลำ)",
            ladder_matched_flex(pending_bet, pending_bet["maker_id"])
        )
        push_flex_async(
            taker_id,
            "จับคู่สำเร็จ (บ่ลำ/ลำ)",
            ladder_matched_flex(pending_bet, taker_id)
        )

        return None

    # ──────────────────────────────────────────────
    # ขั้น 1: B reply โพสต์ บ่ลำ/ลำ ที่เปิดอยู่
    # ──────────────────────────────────────────────
    bet = LADDER_BETS.get(quoted_message_id)
    if not bet or bet.get("status") != "open":
        return None
    ...
    if user_credit_amount(taker) < amount:
        return insufficient_credit_warning(taker, amount, play_text=f"ติดบ่ลำ/ลำ{amount:,}", action="เดิมพันบ่ลำ/ลำ")
...
def settle_ladder_bets_for_round(round_id: str, result_choice: str) -> dict:
    """
    ตัดสินผลแผล บ่ลำ/ลำ ทั้งหมดในรอบนั้นๆ
    result_choice: "บ่ลำ" หรือ "ลำ"
    คืนค่า dict สรุปผล
    """
    matched_count = 0
    maker_win_count = 0
    ...
    target_bets = [
        b for b in LADDER_BETS.values()
        if b.get("round_id") == round_id and b.get("status") == "matched"
    ]
    
    for bet in target_bets:
        matched_count += 1
        bet["result_value"] = result_choice
        bet["winning_choice"] = result_choice
        bet["settled_at"] = now_text()
        ...
        commission = int(profit_earned * commission_rate)
        net_profit = profit_earned - commission
        ...
        # บันทึกกำไรเข้าระบบ (PROFIT)
        if commission > 0:
            add_profit_record(
                round_id=round_id,
                camp_name=bet.get("camp_name", "-"),
                order_no=bet.get("order_no", 0),
                profit_type="ladder_commission",
                amount=commission,
                details=f"หัก 10% บ่ลำ/ลำ ยอด {amount:,} (ผู้ชนะได้รับ {payout:,})"
            )
            
        # ส่งแจ้งเตือนผลรายคนแบบ async
        push_flex_async(
            maker_id,
            f"ผล บ่ลำ/ลำ {bet.get('camp_name') or '-'}",
            oe_result_flex(bet, maker_id) # สามารถใช้ oe_result_flex ได้เพราะโครงสร้างข้อมูลเหมือนกัน
        )
        push_flex_async(
            taker_id,
            f"ผล บ่ลำ/ลำ {bet.get('camp_name') or '-'}",
    ...
    # ยกเลิกแผลค้าง
    _cancel_unmatched_ladder_for_round(round_id, reason=f"สรุปผล {result_choice}")
    _cancel_pending_ladder_for_round(round_id, reason=f"สรุปผล {result_choice}")
    
    save_user_db()
    save_round_backup_db(reason="ladder_settled")
    
    return {
        "matched_count": matched_count,
        "maker_win_count": maker_win_count,
        "taker_win_count": taker_win_count,
        "total_payout": total_payout,
        "total_refund": total_refund
    }


def settle_ladder_bets_all_jow_for_round(round_id: str) -> dict:
    """
    ตัดสินผลแผล บ่ลำ/ลำ ให้ 'จาว' คืนทุนทุกคนในรอบนั้นๆ (กรณีจาว/ยกเลิกฝน)
    """
    matched_count = 0
    total_refund = 0
    ...
    for bet in target_bets:
        ...
        push_flex_async(
            bet["maker_id"],
            f"ผล บ่ลำ/ลำ {bet.get('camp_name') or '-'} (จาว)",
            oe_result_flex(bet, bet["maker_id"])
        )
        push_flex_async(
            bet["taker_id"],
            f"ผล บ่ลำ/ลำ {bet.get('camp_name') or '-'} (จาว)",
            oe_result_flex(bet, bet["taker_id"])
        )
                
    _cancel_unmatched_ladder_for_round(round_id, reason="จาวทุกแผล")
    _cancel_pending_ladder_for_round(round_id, reason="จาวทุกแผล")
    
    save_user_db()
    save_round_backup_db(reason="ladder_settled_all_jow")
...
def _cancel_unmatched_ladder_for_round(round_id: str, reason: str = "รอบปิด"):
    """ยกเลิกโพสต์ บ่ลำ/ลำ ที่ไม่มีคนติด (status='open') และคืนเครดิต"""
    for bet in LADDER_BETS.values():
        if bet.get("round_id") == round_id and bet.get("status") == "open":
            maker_id = bet["maker_id"]
            amount = bet["amount"]
            ...
            push_text_async(
                maker_id,
                f"⚠️ ยกเลิก (บ่ลำ/ลำ) — ไม่มีคนมาติด\n"
                f"เหตุผล: {reason}\n"
                f"คืนเครดิต {amount:,} บาท"
            )
    save_user_db()
...
def _cancel_pending_ladder_for_round(round_id: str, reason: str = "รอบปิด"):
    """ยกเลิกแผล บ่ลำ/ลำ ที่ค้างในระบบทั้งหมดเมื่อล้างระบบ"""
    for bet in LADDER_BETS.values():
        if bet.get("round_id") == round_id and bet.get("status") == "pending_taker":
            maker_id = bet["maker_id"]
            amount = bet["amount"]
            ...
            push_text_async(
                maker_id,
                f"⚠️ ยกเลิก (บ่ลำ/ลำ) — หมดเวลาจับคู่\n"
                f"เหตุผล: {reason}\n"
                f"คืนเครดิต {amount:,} บาท"
            )
    save_user_db()
def _cancel_unmatched_ladder_for_round(round_id: str):
    """ยกเลิกโพสต์ บ่ลำ/ลำ ที่ไม่มีคนติด (status='open') และคืนเครดิต"""
    for bet in LADDER_BETS.values():
        if bet.get("round_id") == round_id and bet.get("status") == "open":
            maker = USERS.get(bet["maker_id"])
            if maker:
                maker["credit"] += bet["amount"]
            bet["status"] = "cancelled"
            bet["cancel_reason"] = "ปิดรอบแล้วไม่มีคนติด"
    save_user_db()


def _cancel_pending_ladder_for_round(round_id: str):
    """ยกเลิกแผล บ่ลำ/ลำ ที่ค้างในระบบทั้งหมดเมื่อล้างระบบ"""
    for bet in LADDER_BETS.values():
        if bet.get("round_id") == round_id and bet.get("status") in {"open", "matched"}:
            # คืนเงินให้ maker
            maker = USERS.get(bet["maker_id"])
            if maker:
                maker["credit"] += bet["amount"]
            # คืนเงินให้ taker (ถ้าจับคู่แล้ว)
            if bet.get("status") == "matched":
                taker = USERS.get(bet["taker_id"])
                if taker:
                    taker["credit"] += bet["amount"]
            bet["status"] = "cancelled"
            bet["cancel_reason"] = "ล้างระบบ/ล้างรอบ"
    save_user_db()
...
def parse_credit_command(text):
    """
    $+ 1 1000 หรือ $+1 1000 หรือ s+1 1000 หรือ s-1 1000
    """
    # รองรับ $, s และเครื่องหมาย +, - และช่องว่างที่ยืดหยุ่น
    m = re.match(r"^([$s])\s*(\+|-)\s*(\d+)\s+(\d+)$", text.strip().lower())
    if not m:
        return None

    return {
        "op": m.group(2),
        "member_no": int(m.group(3)),
        "amount": int(m.group(4)),
    }
...
def get_cancelled_chty_rows_for_user(matches: list, user_id: str):
    ...
    # 4) ดึงจากแผลบ่ลำ/ลำ (LADDER_BETS) ที่ยังไม่ settled
    for bet in LADDER_BETS.values():
        if bet.get("status") in {"open", "matched"} and (bet.get("maker_id") == user_id or bet.get("taker_id") == user_id):
            is_maker = (bet.get("maker_id") == user_id)
            role_text = "โพสต์" if is_maker else "ติด"
            choice_text = bet.get("maker_choice") if is_maker else ("ลำ" if bet.get("maker_choice") == "บ่ลำ" else "บ่ลำ")
            
            status_text = "รอคนติด" if bet.get("status") == "open" else "จับคู่สำเร็จ"
            ...
            rows.append({
                ...
                "order_no": bet.get("order_no"),
                "camp_name": bet.get("camp_name", "-"),
                "detail": f"แผล บ่ลำ/ลำ ({role_text} {choice_text})",
...
def settle_round(result_value: int):
    ...
    # ======================================================
    # ตัดสินบิลคี่/คู่ในรอบนี้ด้วยผลเดียวกัน
    # ======================================================
    settle_odd_even_bets_for_round(current_round_id, result_value)
...
def settle_round_all_jow(reason: str):
    ...
    # ตัดสินบิลคี่/คู่แบบคืนทุนทุกคน
    settle_odd_even_bets_all_jow_for_round(current_round_id, reason)
...
def current_round_report():
    ...
    # นับจำนวนแผลบ่ลำ/ลำในรอบปัจจุบัน
    ladder_matched_count = sum(
        1 for b in LADDER_BETS.values()
        if b.get("round_id") == current_round_id and b.get("status") == "matched"
    )
    ladder_pending_count = sum(
        1 for b in LADDER_BETS.values()
        if b.get("round_id") == current_round_id and b.get("status") == "pending_taker"
    )
    ladder_open_count = sum(
        1 for b in LADDER_BETS.values()
        if b.get("round_id") == current_round_id and b.get("status") == "open"
    )
    ...
    return (
        ...
        f"แผลบ่ลำ/ลำสำเร็จ: {ladder_matched_count}\n"
        f"แผลบ่ลำ/ลำรอยืนยัน: {ladder_pending_count}\n"
        f"แผลบ่ลำ/ลำรอคนติด: {ladder_open_count}"
        f"{pending_text}"
    )
...
def handle_clear_all(event, user_id):
    ...
    with STATE_LOCK:
        ...
        # คืนเครดิตและยกเลิกแผลบ่ลำ/ลำ (LADDER_BETS) ทั้งหมดที่ยังไม่แจ้งผล
        for bet in list(LADDER_BETS.values()):
            if bet.get("status") == "matched":
                amount = int(bet.get("amount", 0) or 0)
                maker = USERS.get(bet.get("maker_id"))
                taker = USERS.get(bet.get("taker_id"))
...
def handle_multi_play_command(event, text: str):
    ...
    เล่น +5ถ500 ชถ500 บังคับเล่น 1000
    ระบบจะแกะออกเป็น 2 แผลย่อย:
    1. +5ถ500 (เล่น 500)
    2. ชถ500 (เล่น 500)
    ยอดบังคับรวม 1000 บาท
    """
    user_id = event.source.user_id
    user = ensure_user_from_event(event)
    
    clean = re.sub(r"\s+", " ", text.strip())
    # ค้นหารูปแบบ "เล่น ... บังคับเล่น <ยอด>"
    m = re.match(r"^เล่น\s+(.+?)\s+บังคับเล่น\s*(\d+)$", clean)
    if not m:
        return None
        
    plays_text = m.group(1)
    forced_total = int(m.group(2))
    
    # แยกแผลย่อยด้วยช่องว่าง
    parts = [p.strip() for p in plays_text.split() if p.strip()]
    if not parts:
        return "❌ รูปแบบคำสั่งไม่ถูกต้อง ไม่มีแผลย่อยให้เล่น"
        
    # ตรวจสอบความถูกต้องของแต่ละแผลย่อยและคำนวณยอดรวม
    parsed_offers = []
    total_amount = 0
    
    for part in parts:
        # พยายาม parse แผลย่อยด้วย parse_offer ปกติ
        offer = parse_offer(part)
        if not offer:
            # พยายาม parse คี่/คู่ หรือ บ่ลำ/ลำ ด้วย
            oe = parse_odd_even_offer(part)
            if oe:
                offer = {"type": "odd_even", "data": oe, "amount": oe["amount"], "raw_text": part}
            else:
                ladder = parse_ladder_offer(part)
                if ladder:
                    offer = {"type": "ladder", "data": ladder, "amount": ladder["amount"], "raw_text": part}
                    
        if not offer:
            return f"❌ แผลย่อย '{part}' รูปแบบไม่ถูกต้อง"
            
        parsed_offers.append(offer)
        total_amount += offer["amount"]
        
    # ตรวจสอบว่ายอดรวมของแผลย่อย ตรงกับยอดบังคับเล่นหรือไม่
    if total_amount != forced_total:
        return f"❌ ยอดรวมของแผลย่อย ({total_amount:,} บาท) ไม่ตรงกับยอดบังคับเล่น ({forced_total:,} บาท)"
        
    # ตรวจสอบเครดิตผู้เล่นว่าพอสำหรับยอดรวมทั้งหมดหรือไม่
    if user_credit_amount(user) < total_amount:
        return insufficient_credit_warning(user, total_amount, play_text=f"เล่นหลายแผล {forced_total:,}", action="เดิมพันหลายแผล")
        
    # ดำเนินการโพสต์แต่ละแผลย่อยเข้าระบบทีละแผล
    success_details = []
    
    with STATE_LOCK:
        for offer in parsed_offers:
            if isinstance(offer, dict) and offer.get("type") == "odd_even":
                # สร้างแผลคี่/คู่
                err = create_odd_even_post(event, offer["data"])
                if err:
                    return f"❌ เกิดข้อผิดพลาดในการสร้างแผล '{offer['raw_text']}': {err}"
                success_details.append(f"• คี่/คู่: {offer['raw_text']}")
                
            elif isinstance(offer, dict) and offer.get("type") == "ladder":
                # สร้างแผลบ่ลำ/ลำ
                err = create_ladder_post(event, offer["data"])
                if err:
                    return f"❌ เกิดข้อผิดพลาดในการสร้างแผล '{offer['raw_text']}': {err}"
                success_details.append(f"• บ่ลำ/ลำ: {offer['raw_text']}")
                
            else:
                # สร้างแผลปกติ
                err = create_post(event, offer)
                if err:
                    return f"❌ เกิดข้อผิดพลาดในการสร้างแผล '{offer['raw_alias']}{offer['amount']}': {err}"
                success_details.append(f"• แผลปกติ: {offer['raw_alias']}{offer['amount']:,}")
                
    # ตอบกลับแอดมิน/ลูกค้าว่าสร้างโพสต์สำเร็จครบถ้วน
    details_str = "\n".join(success_details)
    return (
        f"✅ โพสต์หลายแผลสำเร็จ (รวม {forced_total:,} บาท)\n"
        f"สมาชิก: {user_display_name(user_id)}\n"
...
def handle_message(event):
    ...
    # ลูกค้าโพสต์ เช่น ชล500 / ชถ500
        # 14.5) คำสั่งเล่นหลายแผล (เช่น เล่น +5ถ500 ชถ500 บังคับเล่น 1000)
    if text.strip().startswith("เล่น") and "บังคับเล่น" in text:
        err = handle_multi_play_command(event, text)
        if err:
            reply_text(event.reply_token, err)
        return
    ...
    # ======================================================
    # บ่ลำ / ลำ — เดิมพันบ่ลำ/ลำ
    # เล่นได้เฉพาะตอนเปิดรอบ (ถ้าปิดรอบจะเล่นไม่ได้)
    # ======================================================
    ladder_offer = parse_ladder_offer(text)
    if ladder_offer:
        with STATE_LOCK:
            msg = create_ladder_post(event, ladder_offer)
        if msg:
            reply_problem(event, msg)
        return
    ...
    if confirm_cmd:
        ...
        # ถ้า reply โพสต์บ่ลำ/ลำ
        if quoted_message_id and (
            is_ladder_post_message_id(quoted_message_id)
            or is_ladder_pending_taker_message_id(quoted_message_id)
        ):
            with STATE_LOCK:
                msg = handle_ladder_confirm(event, quoted_message_id, confirm_cmd.get("amount"))
            if msg:
                reply_problem(event, msg)
            return
