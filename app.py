...
BANK_ACCOUNT_NUMBER = "020218276333"
BANK_ACCOUNT_DISPLAY_NUMBER = "020-2-1827633-3"
BANK_ACCOUNT_BANK = "ออมสิน"
BANK_ACCOUNT_NAME = "ธวัชชัย บุญศรี"
...
def bank_account_6_accounts_text() -> str:
    """ข้อความ 6 บัญชี สำหรับแชทส่วนตัว 1-1"""
    return (
        "📌💎บั้งไฟเหล่าเซียน💯💵\n"
        "━━━━━━━━━━━━━━\n\n"
        "🏦 บัญชีรับฝากเงิน\n\n"
        ...
        "─── บัญชีที่ 3 ───\n"
        "🟡 ธนาคาร  : กสิกรไทย\n"
        "🔢 เลขบัญชี : 131-3-94209-7\n"
        "👤 ชื่อบัญชี : บุษบารัตน์ สว่างวงษ์\n\n"
        "─── บัญชีที่ 4 ───\n"
        "🟡 ธนาคาร  : ออมสิน\n"
        "🔢 เลขบัญชี : 020-3-5138303-7\n"
        "👤 ชื่อบัญชี : บุษบารัตน์ สว่างวงษ์\n\n"
        "─── บัญชีที่ 5 ───\n"
        "🟢 ธนาคาร  : กรุงไทย\n"
...
def _build_scoreboard_flex_from_rows(rows, limit: int = 120):
    """Build scoreboard flex: 15 rows/bubble, carousel เดียว ≤12 bubble (~45KB max)
    ถ้า >120 rows จะ truncate ที่ 120
    """
    if not rows:
        return None
    ...
    per_page = 15  # 15×8 bubble = ~45KB carousel เดียว ✅
    ...
    # carousel เดียวทุก bubble — LINE รองรับสูงสุด 12 bubble
    return {"type": "carousel", "contents": all_bubbles}
...
def handle_message(event):
    ...
    if is_scoreboard_command(text) and not (is_private_chat(event) and score_clean == "รายการ"):
        ...
        if flex:
            reply_flex(event.reply_token, "📋 ผลบั้งไฟ", flex)
        else:
