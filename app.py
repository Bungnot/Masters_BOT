...
# คำสั่งเล่นแบบคู่/คี่ (even/odd) — จับคู่เหมือน ชล/ชถ ผลตัดสินจากความเป็นคู่/คี่ของตัวเลขผล
EVEN_ALIASES = ["คู่"]
ODD_ALIASES  = ["คี่"]
ALL_EVEN_ODD_ALIASES = EVEN_ALIASES + ODD_ALIASES

# คำสั่งเล่นแบบ ลำ/บ่ลำ — จับคู่เหมือน ชล/ชถ ผลตัดสินด้วย "สรุปผล ลำ/บ่ลำ" แยกต่างหาก
# เล่นได้เฉพาะขณะเปิดรอบเท่านั้น
LAM_ALIASES    = ["ลำ"]
BOLAM_ALIASES  = ["บ่ลำ"]
ALL_LAM_ALIASES = sorted(BOLAM_ALIASES + LAM_ALIASES, key=len, reverse=True)  # บ่ลำ ก่อน ลำ
...
def normalize_side(alias: str) -> str:
    ...
    if alias in ONLY_LOSE_ALIASES:
        return "ช่างแพ้"
    if alias in EVEN_ALIASES:
        return "คู่"
    if alias in ODD_ALIASES:
        return "คี่"
    if alias in LAM_ALIASES:
        return "ลำ"
    if alias in BOLAM_ALIASES:
        return "บ่ลำ"
    return ""
...
def is_even_odd_side(side: str) -> bool:
    """ตรวจว่าเป็น side คู่/คี่"""
    return side in {"คู่", "คี่"}


def is_lam_side(side: str) -> bool:
    """ตรวจว่าเป็น side ลำ/บ่ลำ"""
    return side in {"ลำ", "บ่ลำ"}
...
def parse_even_odd_offer(text: str):
    ...
    """
    clean = compact_play_command_text(text)
    even_odd_pattern = "|".join(re.escape(x) for x in sorted(ALL_EVEN_ODD_ALIASES, key=len, reverse=True))
    m = re.match(rf"^({even_odd_pattern})(\d+)$", clean)
    if not m:
        return None
    alias = m.group(1)
    amount = int(m.group(2))
    ...
    if not maker_side:
        return None
    return {
        "plus": 0, "amount": amount, "raw_alias": alias,
        "maker_side": maker_side,
        "custom_price_min": None, "custom_price_max": None,
        "is_custom_price": False, "only_when_no_price": False,
        "price_adjust_target": None, "price_adjust_min": None, "price_adjust_max": None,
        "is_two_digit_price": False, "two_digit_min_token": None, "two_digit_max_token": None,
        "market_type": "even_odd",
    }
...
def parse_lam_offer(text: str):
    ...
    """
    clean = compact_play_command_text(text)
    lam_pattern = "|".join(re.escape(x) for x in ALL_LAM_ALIASES)
    m = re.match(rf"^({lam_pattern})(\d+)$", clean)
    if not m:
        return None
    alias = m.group(1)
    amount = int(m.group(2))
...
def create_post(event, offer):
    ...
    maker_side = offer.get("maker_side", "")
    market_type = offer.get("market_type")

    # ลำ/บ่ลำ — เล่นได้เฉพาะขณะเปิดรอบเท่านั้น
    if market_type == "lam":
        if STATE.get("round_id") is None:
    ...
    # คู่/คี่ — เล่นได้ก่อนเปิด ขณะเปิด และหลังปิด (แต่ก่อนแจ้งผล)
    elif market_type == "even_odd":
        if STATE.get("round_id") is None:
    ...
    POSTS[post_id] = {
        ...
        "status": "open",
        "created_at": now_text(),
        "market_type": offer.get("market_type") or "normal",  # "normal" / "even_odd" / "lam"
    }
...
def settle_round(result_value: int):
    ...
    target_matches = [
        ...
        and not is_lam_side(m.get("maker_side", ""))  # ลำ/บ่ลำ รอ สรุปผล แยก
    ]
    ...
    for match in target_matches:
        ...
        if is_even_odd_side(maker_side):
            # คู่/คี่: ตัดสินจากความเป็นเลขคู่/คี่ของผล
            result_parity = "คู่" if int(result_value) % 2 == 0 else "คี่"
            winning_side = result_parity
            match_price_text = f"คู่/คี่ (ผล {result_value} = {result_parity})"
        elif STATE.get("price_mode") == "no_price" and not match.get("is_custom_price"):
            winning_side = "จาว"
...
def settle_lam_matches(winning_side: str):
    """
    สรุปผลลำ/บ่ลำ ทุก match ในรอบปัจจุบันที่ maker_side เป็น ลำ หรือ บ่ลำ
    - winning_side: "ลำ" หรือ "บ่ลำ"
    - หักค่า% จากคนชนะ เหมือน ชล/ชถ ทุกอย่าง
    ...
    target_matches = [
        ...
        and is_lam_side(m.get("maker_side", ""))
    ]
    ...
    # คืนทุนโพสต์ที่ยังไม่ถูกจับคู่ (remaining_amount > 0)
    cancelled_posts = 0
    for post in list(POSTS.values()):
        if post.get("round_id") != current_round_id:
            continue
        if not is_lam_side(post.get("maker_side", "")):
            continue
        ...
        if remaining > 0:
            ...
            post["status"] = "cancelled"
            post["cancel_reason"] = "lam_settled_no_match"
            post["cancelled_at"] = now_text()
            cancelled_posts += 1
            push_text_async(
                post.get("maker_id"),
                f"💎 คืนทุนโพสต์ลำ/บ่ลำ ค่าย {camp_name}\n"
                f"เล่น: {post.get('maker_side')} {remaining:,} (ไม่มีคู่) | คืนทุนเต็มจำนวน"
            )

    if round_commission_total > 0:
        add_profit_record(
            current_round_id, camp_name, winning_side,
            round_commission_total, [], f"ลำ/บ่ลำ"
        )

    save_user_db()

    # ส่ง Flex สรุปผลให้แต่ละคน (reuse result_summary_flex เดิม)
    for uid, rows in user_rows.items():
        push_flex_async(
            uid,
            f"ผลลำ/บ่ลำ {camp_name}",
            result_summary_flex(uid, rows, user_net.get(uid, 0))
        )
    ...
    lines = [
        f"✅ สรุปผลลำ/บ่ลำ ค่าย {camp_name}",
        f"ผล: {winning_side}",
        f"บิลที่คิดผล: {settled_count}",
    ]
    if cancelled_posts:
        lines.append(f"โพสต์ที่คืนทุน (ไม่มีคู่): {cancelled_posts}")
    if round_commission_total:
        lines.append(f"ค่าคอมมิชชั่นรวม: {round_commission_total:,}")
    return "\n".join(lines)
...
def handle_lam_result_command(event, winning_side: str):
    ...
    open_posts = [
        ...
        and is_lam_side(p.get("maker_side", ""))
        and p.get("status") in {"open", "closed"}
    ]
    if not pending and not open_posts:
        return "ไม่มีรายการลำ/บ่ลำในรอบนี้"

    return settle_lam_matches(winning_side)
...
def handle_message(event):
    ...
    if camp_name:
        ...
        # กันเปิดรอบใหม่ถ้ายังมีบิลลำ/บ่ลำ matched ค้างอยู่ในรอบปัจจุบัน
        current_round_id = STATE.get("round_id")
        if current_round_id:
            unsettled_lam = [
                m for m in MATCHES.values()
            ...
            if unsettled_lam:
                ...
                reply_text(
                    event.reply_token,
                    f"❌ เปิดรอบใหม่ไม่ได้\n\n"
                    f"ค่าย {camp} ยังมีบิลลำ/บ่ลำที่ยังไม่ได้สรุปผลอยู่ {len(unsettled_lam)} บิล\n\n"
                    f"กรุณาสรุปผลก่อน:\n"
                    f"สรุปผล ลำ  หรือ  สรุปผล บ่ลำ"
                )
                return
    ...
    # เล่นได้ก่อนเปิด ขณะเปิด และหลังปิด (แต่ก่อนแจ้งผล)
    even_odd = parse_even_odd_offer(text)
    if even_odd:
        msg = create_post(event, even_odd)
        if msg:
            reply_problem(event, msg)
        return

    # ลูกค้าโพสต์ลำ/บ่ลำ เช่น ลำ300 / บ่ลำ500
    # เล่นได้เฉพาะขณะเปิดรอบ
    lam = parse_lam_offer(text)
    if lam:
        msg = create_post(event, lam)
        if msg:
            reply_problem(event, msg)
        return

    # แอดมินสรุปผลลำ/บ่ลำ (แยกจาก แจ้งผล ตัวเลขปกติ)
    lam_result = parse_lam_result_command(text)
    if lam_result:
        with STATE_LOCK:
            msg = handle_lam_result_command(event, lam_result)
        if msg:
            reply_text(event.reply_token, msg)
        return
